...
export class Renderer {
  ...
  render(world: World, frameDt: number, cameraActive: boolean, alpha = 1): void {
    ...
    this.applyRoomStyle(level, decompositionFor(world));
   ...
   *
   * **`decomp` comes from the campaign and overrides the room, not the other way
   * round.** It is applied last and unconditionally, so a room cannot author its
   * way out of the site's condition and cannot author its way into it either.
   */
  private applyRoomStyle(level: LevelDef | null, decomp: Decomposition): void {
    const id = `${level?.shell ? level.id : ''}:${decomp.key}`;
    if (id === this.roomStyleId) return;
    this.roomStyleId = id;
    const style = level?.shell ? ({ ...SHELL, ...level.shell } as typeof SHELL) : SHELL;
    // ?look= wins over everything — it exists so an eye can compare the smoke
    // systems in isolation, and a comparison the campaign can repaint is not
    // a comparison.
    for (const shell of this.shells) {
      shell.setStyle({ ...style, ...decomp.dials, ...(LOOK_OVERRIDE ?? {}) });
    }
  }
 ...
 * ruins deteriorating is a thing that happens over six nights, so the same
 * corridor must be sound early and gone late, which is only legible if the room
 * is not also shouting its own permanent condition. Rooms differentiate on churn
...
const LOOK_OVERRIDE: Partial<import('./gfx/structure').ShellStyle> | null = (() => {
  try {
    ...
    return raw ? (LOOKS[raw] ?? null) : null;
  } catch {
    return null;
  }
})();

const DECOMP_OVERRIDE: Decomposition['dials'] | null = (() => {
  try {
    if (typeof window === 'undefined') return null;
...
function decompositionFor(world: World): Decomposition {
  if (DECOMP_OVERRIDE) return { key: 'override', dials: DECOMP_OVERRIDE };
  const k = Math.max(0, Math.min(1, world.config.siteDecay ?? 0));
  // The key is quantised because the uniforms are only rewritten when it changes,
  // and three floats in a key would rewrite them whenever one moved in its last
  // decimal place. The ramp is per-rung, so in practice this is constant in-run.
  return { key: k.toFixed(3), dials: decompositionDials(k) };
}
