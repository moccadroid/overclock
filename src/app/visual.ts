...
export const SHELL = {
  ...
  fray: 0,
  /**
   * STORY-AND-TONE §8.3 — the decomposition, and there is exactly one: the
   * flow. The mass's coverage becomes a domain-warped FBM field — solid deep
   * inside (guaranteed: the base layer floors the collider and only ever
   * roils outward), breaking into genuinely flowing smoke with no
   * iso-contour anywhere. See ShellStyle.flow in gfx/structure.ts for the
   * system and structure.test.ts for the frozen approved recipe.
   *
   * Driven by the campaign (siteDecay) at runtime — see decompositionFor in
   * renderer.ts; the stored zero is the sound baseline. 0 is crisp sound
   * geometry, 1 is the full approved roil, and the ramp between is the
   * site's six-night deterioration.
   */
  flow: 0,
  /**
